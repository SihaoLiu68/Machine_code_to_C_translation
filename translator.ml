...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  match tail with
  | PT_nt ("TT", []) -> lhs
  | PT_nt ("TT", [ao; term; term_tail]) -> AST_binop (ast_ize_op ao, lhs, ast_ize_expr_tail (ast_ize_expr term) (term_tail))
  | PT_nt ("FT", []) -> lhs
  | PT_nt ("FT", [mo; factor; factor_tail]) -> AST_binop (ast_ize_op mo, lhs, ast_ize_expr_tail (ast_ize_expr factor) (factor_tail))
...
let rec translate (ast:ast_sl) : string * string =
  ...
  let str = (String.concat "" ["//C CODE:\n\n#include <stdio.h>\n";
                         "#include <stdlib.h>\n\n";
                         "double getreal() { double a; char ch; if(scanf(\"%d\", &a) == 0) { printf(\"Error: cannot enter non-numeric input.\\n\"); exit(1); } else if(scanf(\"%c\", &ch) == EOF) { printf(\"Error: unexpected end of input.\\n\"); exit(1); } return a; }\n";
                         "void putreal(double a) { printf(\"%d\\n\", a); }\n";
                         "double divide(double x, double y) { if(y == 0) { printf(\"Error: cannot divide by 0.\\n\"); exit(1); } return x / y; }\n\n";
                         "double main() {\n";
                         str;
                         "}\n"]) in
  print_string str;
