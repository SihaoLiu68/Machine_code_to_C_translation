...
let rec make_used (old_vl:decl_list) (new_vl:decl_list) (var:decl_var) =
  ...
  | h::t -> if (String.equal (id_name h) (id_name var))
            then make_used (t) (List.append (new_vl) ([DECL_var((id_name var), 1)])) (var)
            else make_used (t) (List.append (new_vl) ([h])) (var)
  | [] -> new_vl;;

(* return all unused variables in the program*)
let rec check_unused (vl:decl_list) (str:string) : string =
  match vl with
  | h::t -> if ((check_used h) = 0)
            then ((check_unused (t)) (String.concat " unused variable " [str; id_name h; "\n"]))
            else ((check_unused (t)) (str))
  | [] -> str;;
...
and translate_expr (ast : ast_e) (vl : decl_list) : string * decl_list =
  ...
  | AST_id (id)
      ->if check_decl (vl) (id)
        then id, (make_used (vl) ([]) (DECL_var(id,1)))
        else (Printf.printf "%s is not found in declaration \n" id; (id, (make_used (vl) ([]) (DECL_var(id,1)))))
