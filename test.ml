...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =    (* TT or FT *)
  ...
  | PT_nt ("FT", [])
        -> lhs 
  | PT_nt ("FT", [PT_nt ("mo", [PT_term mo]); factor; factor_tail]) when member mo ["*"; "/"]
        -> AST_binop (mo, lhs, ast_ize_expr_tail (ast_ize_expr factor) factor_tail)
  | _ -> raise (Failure "malformed parse tree in ast_ize_expr_tail")

and ast_ize_C (c:parse_tree) : ast_c =
  match c with
  | PT_nt ("C", [expr1; PT_nt ("rn", [PT_term rn]); expr2]) when member rn ["="; "<>"; "<"; ">"; "<="; ">="]
        -> (rn, ast_ize_expr expr1, ast_ize_expr expr2)
...
(* for convience, make a new type here *)
type decl_list = decl_var list
and decl_var = DECL_var of (string * int * int);;

(* return the id name  *)
let id_name (v:decl_var) : string =
  match v with
  | DECL_var(id_str, used, declared) -> id_str;;

(* return the used state of the declared variable *)
let check_used (v:decl_var) : int =
  match v with
  | DECL_var(id_str, used, declared) -> used;;

(* return the declare state of the declared variable *)
let check_declared (v:decl_var) : int =
  match v with
  | DECL_var(id_str, used, declared) -> declared;;

(* check if a variable is declared *)
let rec check_decl (vl:decl_list) (var:string) =
  match vl with 
  | h::t -> if (String.equal (var) (id_name h)) then true else check_decl (t) (var)
  | [] -> false;;

(* changing the used state of a variable *)
let rec make_used (old_vl:decl_list) (new_vl:decl_list) (var:decl_var) =
  match old_vl with
  | h::t -> if (String.equal (id_name h) (id_name var))
            then (make_used (t) (List.append (new_vl) ([DECL_var((id_name var), check_used var, check_declared var)])) (var))
            else make_used (t) (List.append (new_vl) ([h])) (var)
  | [] -> new_vl;;

(* return all unused variables in the program*)
let rec check_unused_undeclared (vl:decl_list) (str:string) : string =
  match vl with
  | h::t -> if ((check_used h) = 0)
            then ((check_unused_undeclared (t)) (String.concat "" [ str; id_name h; " is unused \n"]))
            else if ((check_declared h) = 0)
            then ((check_unused_undeclared (t)) (String.concat "" [ str; id_name h; " is undeclared \n"]))
            else ((check_unused_undeclared (t)) (str))
(* 
            if ((check_used h) = 0)
            then ((check_unused_undeclared (t)) (String.concat "\nunused variable: " [str; id_name h; "    "]))
            else if ((check_declared h) = 0)
            then ((check_unused_undeclared (t)) (String.concat "\nundeclared variable: " [str; id_name h; "    "]))
            else ((check_unused_undeclared (t)) (str)) *)

            (* if ((check_used h) = 0)
            then ((check_unused_undeclared (t)) (String.concat "" [ id_name h; " is unused \n"]))
            else if ((check_declared h) = 0)
            then ((check_unused_undeclared (t)) (String.concat "" [ id_name h; " is undeclared \n"]))
            else ((check_unused_undeclared (t)) (str)) *)
  | [] -> str;;

(* necessary functions and library *)
...
let epilogue = "return 0; \n}   ";;

let rec translate (ast:ast_sl) : string * string =
  let (main_prog, vl) = translate_sl ast [] in
  let main_prog = (String.concat "" [
                        prologue;
                        main_prog;
                        epilogue]) in

  (check_unused_undeclared (vl) (""), main_prog)
  (* (check_undeclared (vl) (""), main_prog) *)

and translate_sl (ast : ast_sl) (vl : decl_list) : string * decl_list =
  match ast with
...
and translate_s (ast : ast_s) (vl : decl_list) : string * decl_list =
  match ast with
  | AST_assign (id, expr)
  ...
  | AST_if (cond, sl)
      -> translate_if cond sl vl
  | AST_while (cond, sl)
      -> translate_while cond sl vl 
  | _ -> "translate_s error", vl

and translate_assign (id:string) (expr:ast_e) (vl:decl_list) : string * decl_list =
  if check_decl (vl) (id)
  then (let (main_prog, new_vl) = translate_expr expr vl in
        String.concat "" [id; " = "; main_prog; ";\n"], new_vl)
  else (let (main_prog, new_vl) = translate_expr expr vl in
        String.concat "" ["double "; id; " = "; main_prog; ";\n"], (List.append (new_vl) ([DECL_var(id, 0, 1)])))

and translate_read (id:string) (vl:decl_list) : string * decl_list =
  String.concat "" ["double "; id; " = "; "getreal();\n"], (List.append (vl) ([DECL_var(id, 0, 1)]))

and translate_write (expr:ast_e) (vl:decl_list) : string * decl_list =
    let (main_prog, new_vl) = translate_expr expr vl in
    String.concat "" ["putreal ("; main_prog; ");\n"], new_vl

and translate_if (cond: ast_c) (statement_list: ast_sl ) (vl : decl_list) : string * decl_list =
  let (str1, new_vl) = translate_cond cond vl in
  let (str2, new_vl) = translate_sl statement_list new_vl in
  String.concat "" ["if ("; str1; ") {\n"; str2; "}\n"], new_vl 

and translate_while (cond: ast_c) (statement_list: ast_sl ) (vl : decl_list) : string * decl_list = 
  let (str1, new_vl) = translate_cond cond vl in
  let (str2, new_vl) = translate_sl statement_list new_vl in
  String.concat "" ["while ("; str1; ") {\n"; str2; "}\n"], new_vl

and translate_expr (ast : ast_e) (vl : decl_list) : string * decl_list =
  match ast with 
  | AST_num (num)
      -> num, vl
  | AST_id (id)
      ->if check_decl (vl) (id)
        then id, (make_used (vl) ([]) (DECL_var(id,1,1)))
        else id, (List.append (vl) ([DECL_var(id, 1, 0)]))
        (* else raise (Failure (String.concat "" ["\n ------------error------------- \n no declaration of : "; id; " is found. \n ------------error------------- \n\n"])) *)
...
and translate_cond (ast : ast_c) (vl : decl_list) : string * decl_list = 
  match ast with
  | (rn, left_expr, right_expr)
         -> let (str1, new_vl) = translate_expr left_expr vl in
            let (str2, new_vl) = translate_expr right_expr new_vl in 
            if rn = "="
            then String.concat "" ["("; str1; " "; "=="; " "; str2; ")"], new_vl
            else if rn = "<>"
                 then String.concat "" ["("; str1; " "; "!="; " "; str2; ")"], new_vl
                 else String.concat "" ["("; str1; " "; rn; " "; str2; ")"], new_vl
...
let compile (code:string) (program_name:string) =
  try
    ...
    let ofile = open_out ofile_name in
      ...
      close_out ofile;
      "\n-----------------Warnings of " ^ ofile_name ^ "-----------------\n" ^ errs ^ "---------------------------------------------\n" ^ "Code written to " ^ ofile_name ^ "\n\n"
